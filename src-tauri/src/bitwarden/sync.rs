...
pub async fn run_sync(
    ...
    let folder = remote.folders.iter().find(|f| {
        f.name
            .as_ref()
            .map_or(false, |n| n.to_lowercase() == folder_name_lower)
    });
    ...
    let has_legacy = remote.folders.iter().any(|f| {
        f.name
            .as_ref()
            .map_or(false, |n| n.to_lowercase() == "sshspan")
    });
    let has_keys_folder = remote.folders.iter().any(|f| {
        f.name
            .as_ref()
            .map_or(false, |n| n.to_lowercase() == keys_lower)
    });
    ...
    let servers_folder = remote.folders.iter().find(|f| {
        f.name
            .as_ref()
            .map_or(false, |n| n.to_lowercase() == servers_lower)
    });
    ...
    // Build remote maps
    let remote_ssh: Vec<_> = remote
        .ciphers
        .iter()
        .filter(|c| c.cipher_type == 5 && c.deleted_date.is_none() && c.organization_id.is_none())
        .collect();

    // Decrypt remote fingerprints for matching
    let mut fp_by_cipher_id = std::collections::HashMap::new();
    for c in &remote_ssh {
        if let Some(ref fp_enc) = c
            .ssh_key
            .as_ref()
            .and_then(|sk| sk.key_fingerprint.as_ref())
        {
    ...
    for row in &local_keys {
        ...
        let cipher = row
            .bitwarden_id
            .as_ref()
            .and_then(|bw_id| remote_ssh.iter().find(|c| c.id == *bw_id))
            .or_else(|| {
                // Fingerprint match
                remote_ssh.iter().find(|c| {
                    fp_by_cipher_id
                        .get(&c.id)
                        .map_or(false, |fp| *fp == row.fingerprint_sha256)
                })
            });
        ...
        // Compare timestamps
        let local_changed =
            row.updated_at.timestamp_millis() > (row.bitwarden_updated_at.unwrap_or(0));
        let remote_changed = cipher
            .revision_date
            .as_ref()
            .and_then(|rd| chrono::DateTime::parse_from_rfc3339(rd).ok())
            .map_or(false, |rd| {
                rd.timestamp_millis() > (row.bitwarden_revision_ts.unwrap_or(0))
            });
    ...
    for cipher in &remote_ssh {
        ...
        let pub_key = pub_key_enc
            .map(|e| client.decrypt_field(e))
            .transpose()
            .unwrap_or_default();
        let fp = fp_enc
            .map(|e| client.decrypt_field(e))
        ...
        let public_openssh = crate::crypto::keys::export_public_key(
            &key_data,
        ...
        let key_record = KeyRecord {
            ...
            public_key: if pub_key.is_some() {
                pub_key.unwrap()
            } else {
                public_openssh
            },
            private_key_encrypted: sealed,
            fingerprint_sha256: if fp.is_some() {
                fp.unwrap()
            } else {
                fingerprint
            },
    ...
    let remote_logins: Vec<_> = remote
        .ciphers
        .iter()
        .filter(|c| {
            c.cipher_type == 1
    ...
    for sv in &local_servers {
        let cipher = sv
            .bitwarden_id
            .as_ref()
            .and_then(|bw_id| remote_logins.iter().find(|c| c.id == *bw_id));

        let cipher = match cipher {
            Some(c) => c,
            None => {
                match push_local_server(&mut client, sv, &servers_folder_id, db, vault_password)
                    .await
                {
                    Ok(()) => {
                        servers_pushed += 1;
                    ...
                    Err(e) => {
                        errors.push(serde_json::json!({"server": sv.name, "error": e.to_string()}));
                    }
                }
                continue;
            }
        ...
        let local_changed =
            sv.updated_at.to_rfc3339() > sv.bitwarden_updated_at.clone().unwrap_or_default();
        let remote_changed = cipher.revision_date.as_ref().map_or(false, |rd| {
            rd.as_str()
                > sv.bitwarden_revision_ts
                    .clone()
                    .unwrap_or_default()
                    .as_str()
        });
...
async fn push_local_key(
    ...
    let key_data = crate::crypto::keys::PrivateKeyData::new(
        key_type,
        private_bytes,
        public_bytes,
        row.comment.clone(),
    );
    let ossh_private = crate::crypto::keys::export_private_key(
        &key_data,
        crate::crypto::keys::KeyFormat::OpenSsh,
        None,
    )?;
    let ossh_public =
        crate::crypto::keys::export_public_key(&key_data, crate::crypto::keys::KeyFormat::OpenSsh)?;
    ...
    let notes_enc = if notes_plain.is_empty() {
        None
    } else {
        Some(client.encrypt_field(&notes_plain)?)
    };
...
async fn pull_remote_item(
    ...
    let ssh = cipher
        .ssh_key
        .as_ref()
        .ok_or_else(|| anyhow::anyhow!("No sshKey block"))?;
    let priv_enc = ssh
        .private_key
        .as_ref()
        .ok_or_else(|| anyhow::anyhow!("No privateKey"))?;
    ...
    if let Some(kd) = key_data {
        ...
        updated.bitwarden_revision_ts = cipher
            .revision_date
            .as_ref()
            .and_then(|rd| chrono::DateTime::parse_from_rfc3339(rd).ok())
                    ...
                    Ok(v) => v
                        ...
                        .map(|arr| {
                            arr.iter()
                                .filter_map(|item| {
                                    let id =
                                        item.get("id").and_then(|i| i.as_str()).map(String::from);
                                    let path =
                                        item.get("path").and_then(|p| p.as_str()).map(String::from);
                                    match (id, path) {
                                        (Some(id), _)
                                            if db.get_category(&id).ok().flatten().is_some() =>
                                        {
                                            Some(id)
                                        }
                                        (_, Some(path)) => db
...
fn parse_openssh_public_line(line: &str) -> Result<Vec<u8>> {
    ...
    let b64 = line
        .split_whitespace()
        .nth(1)
        .ok_or_else(|| anyhow::anyhow!("Malformed stored public key"))?;
    base64ct::Base64::decode_vec(b64)
        .map_err(|e| anyhow::anyhow!("Malformed stored public key: {e}"))
}
    ...
    let password_enc = match &sv.saved_password {
        Some(sealed) => {
            ...
            if plain_str.is_empty() {
                ...
                Some(client.encrypt_field(&plain_str)?)
            }
        }
        None => None,
    };
...
fn parse_server_notes(
    client: &BitwardenClient,
    cipher: &crate::bitwarden::SyncCipher,
    db: &Database,
) -> (Option<String>, Option<String>, Option<String>) {
    let Some(notes_enc) = cipher.notes.as_ref() else {
        return (None, None, None);
    };
    let Ok(plain) = client.decrypt_field(notes_enc) else {
        return (None, None, None);
    };
    let Ok(v) = serde_json::from_str::<serde_json::Value>(&plain) else {
        return (None, None, None);
    };
    let Some(ss) = v.get("sshspan") else {
        return (None, None, None);
    };
    let auth = ss
        .get("authMethod")
    ...
    let key_id = key_fp
        ...
        .and_then(|fp| {
            db.list_keys()
                .ok()?
                .into_iter()
                .find(|k| &k.fingerprint_sha256 == fp)
                .map(|k| k.id)
        })
