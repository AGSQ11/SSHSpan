...
puttyParser.js parses and writes PuTTY private key files (`.ppk`) for RSA, Ed25519 and ECDSA. **v3** (current) derives key material with Argon2 (Node's built-in `argon2Sync`), splitting the 80-byte tag into AES key / CBC IV / MAC key. **v2** (PuTTY 0.52–0.74) is accepted for import only: it uses `SHA1(seq‖passphrase)` for the cipher key, a zero IV, and `SHA1("putty-private-key-file-mac-key"‖passphrase)` as the HMAC-SHA-1 key. In both cases the integrity check is verified timing-safely over the DECRYPTED private blob before any key material is returned, so a wrong passphrase or tampered file never yields a usable key. Export always writes v3, so importing a v2 key upgrades it. v3 Argon2 parameters read from a file are validated, and implausible memory requests are refused.
